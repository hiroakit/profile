...
- On first launch, Codespaces runs =./scripts/devcontainer-check.sh= which:
  ...
  - applies the Home Manager configuration for =devcontainer@devcontainer= (user =vscode=)
...
- On Linux (Codespaces), =install.sh= installs Nix (if needed) and applies the Home Manager configuration for =devcontainer@devcontainer=.
